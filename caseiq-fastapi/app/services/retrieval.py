...
def _judicial_status_dict(status, case_name, citation, court, decided_on, scope_note) -> dict | None:
    if status is None:
        return None
    return {
...
async def get_section_as_of(
    ...
    Returns None if no in-force version exists at `as_of`, OR if the
    in-force version is struck down (K2 hard rule -- a struck-down section
    is not "found", full stop, not merely flagged).
    """
    ...
    stmt = (
        ...
        .where(Act.act_code == act_code, SectionVersion.section_number == section_number,
               _in_force(as_of), _not_struck_down())
    )
    ...
    return {
        ...
        "judicial_status": _judicial_status_dict(j_status, j_case, j_citation, j_court, j_decided, j_scope),
    }
...
async def semantic_search(
    ...
    stmt = (
        ...
        .where(_in_force(as_of), _not_struck_down(), SectionVersion.embedding.is_not(None))
    )
    ...
    for sv, act_code, dist, j_status, j_case, j_citation, j_court, j_decided, j_scope in rows:
        ...
        if similarity < settings.RAG_MIN_SIMILARITY:
            continue
        results.append(_serialise(sv, act_code, round(similarity, 4),
                                   _judicial_status_dict(j_status, j_case, j_citation, j_court, j_decided, j_scope)))
...
async def keyword_search(
    ...
    stmt = (
        ...
        .where(_in_force(as_of), _not_struck_down(), or_(*clauses))
    )
    if incident_date is not None:
        stmt = stmt.where(Act.act_code.in_(acts_for_incident_date(incident_date)))
    ...
    rows = (await db.execute(stmt)).all()
    return [
        _serialise(sv, act_code, None,
                   _judicial_status_dict(j_status, j_case, j_citation, j_court, j_decided, j_scope))
        for sv, act_code, j_status, j_case, j_citation, j_court, j_decided, j_scope in rows
    ]


def _serialise(sv: SectionVersion, act_code: str, similarity: float | None, judicial_status: dict | None) -> dict:
    return {
        "act": act_code,
